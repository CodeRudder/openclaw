  ...
  const handlePost = async (
    ...
    if (kind !== "direct") {
      ...
      const groupHints = isQueuedMessage
        ? `\n\n[群聊消息处理提示] 这是队列合并消息。忽略其它Agent的分析内容，只关注@你的原始消息。快速判断是否需要回复。不需要回复时仅输出NO_REPLY。需要回复时必须使用message tool发送消息，消息内容必须以[GROUP-CHAT]开头并@接收Agent。发送完消息后继续处理未完成的任务。\n\n`
        : `[群聊消息处理提示] 忽略消息中其它Agent的分析过程，只关注与你相关的@消息或问题。不需要回复时仅输出NO_REPLY。需要回复时必须使用message tool发送消息，消息内容必须以[GROUP-CHAT]开头并@接收Agent。发送完消息后继续处理未完成的任务。\n\n`;
